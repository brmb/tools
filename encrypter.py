...
def encrypt_file(file_path, key):
    try:
        ...
        # Check if the file is hidden by checking if its name starts with '.'
        if os.path.basename(file_path).startswith('.'):
            try:
                subprocess.run(["chattr", "+i", file_path], check=True)
                print(f"Successfully set chattr +i on hidden file: {file_path}")
            except subprocess.CalledProcessError:
                # If chattr fails, skip setting it but continue with the script
                print(f"Could not set chattr +i on: {file_path}, skipping...")
